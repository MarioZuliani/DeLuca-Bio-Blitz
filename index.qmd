...
# Common fungal classes (case-insensitive)
fungi_classes <- c(
  "agaricomycetes","ascomycetes","pezizomycetes","sordariomycetes",
  "leotiomycetes","lecanoromycetes","dothideomycetes","eurotiomycetes",
  "saccharomycetes","tremellomycetes","pucciniomycetes","ustilaginomycetes",
  "exobasidiomycetes"
)

# Protozoa groups used in the BioBlitz species metadata (case-insensitive)
protozoa_classes <- c(
  "euglenida","myxomycetes"
)
...
kingdom_of <- function(cls) {
  ...
  if (x %in% fungi_classes)       return("Fungi")
  if (x %in% protozoa_classes)    return("Protozoa")
  return("Animals")
}
...
kingdoms <- intersect(c("Plants", "Animals", "Fungi", "Protozoa"), sort(unique(na.omit(meta$Kingdom))))
for (kg in kingdoms) {
